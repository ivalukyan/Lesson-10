...
float amplify (int k, float &power) {return power*k;}

void show_amplify(Transmitter _transmitter[]){
    int k = 3;
    for (int i  = 0; i < 2; ++i){
        cout << endl << "Amplify: " << amplify(k, _transmitter[i].power);
    }
    cout << endl;
}
